...
class LUH_CKAN_API_ParserProfile(DatasetParser):
    ...
    def __init__(self):
        ...
        self.mappings_file = os.path.join(self.mappings_path, 'LUH_profile_mapping.json')
    ...
    def get_all_datasets_dicts(self):
        ...
        for ds in ds_list:
            ...
            ds_data = self.adjust_dataset_dict_with_mapping(ds_data)
            ds_array[i] = ds_data
            i += 1
 #           if i == 3:
 #               break
    ...
    def adjust_dataset_dict_with_mapping(self, ds_dict):
    ...
    def execute_before_insert_dataset(self, remote_dataset):
        ...
        '''
        # Now implemented as a mapping function.
    ...
    # Define Mapping custom functions
    def _addPrefix(self, value, params):
        prefix = params.get("prefix", "")
        return f"{prefix}{value}"
    
    def _buildLUHUrl(self, value, params):
        domain = params.get("domain", "https://data.uni-hannover.de")
        return f"{domain}/dataset/{value}"
    
    def _excludeDownloadAll(self, resource, params):
        if "downloadall_datapackage_hash" in resource or "downloadall_metadata_modified" in resource:
            return False
        return True
   
    def _process_LUH_authors(self, value, params):
        ...
        for author in authors:
            ...
            extra_author = {"author": author["lastName"] + ', ' + author["firstName"],
                            "familyName": author["lastName"],
                            "givenName": author["firstName"],
                            "orcid": ""}
            authors_list.append(extra_author)
