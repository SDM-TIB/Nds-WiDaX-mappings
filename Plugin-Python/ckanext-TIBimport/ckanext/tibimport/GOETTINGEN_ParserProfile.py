...
# Shared state used by kgcreation to measure step 3 and step 4 separately.
# kgcreation reads insert_start_time at the beginning of after_create:
#   step3 elapsed = time.time() - insert_start_time  (CKAN DB save)
#   step4 elapsed = time of create_dataset_in_LDM()  (KG creation)
_profiling_context = {
    'enabled': False,
    'logger': None,
...
class GOETTINGEN_ParserProfile(DatasetParser):
    ...
    def __init__(self):
        ...
        self.mappings_path = os.path.join(self.this_file_path, 'mappings/')
        self.mappings_file = os.path.join(self.mappings_path, 'GOE_profile_mapping.json')
        ...
        custom_functions = {
            "extractDOI": self._extractDOI,
            "extractYear": self._extractYear,
            "buildGoettingenName": self._buildGoettingenName,
            "processGOEPublisher": self._processGOEPublisher,
            "processGOESubjectAreas": self._processGOESubjectAreas,
            "processGOERelatedPublications": self._processGOERelatedPublications,
            "processGoettingenKeywords": self._processGoettingenKeywords,
            "processGoettingenCreators": self._processGoettingenCreators,
            "getGoettingenOrganization": self._get_goettingen_organization
            # "buildLUHUrl": self._buildLUHUrl,
            # "excludeDownloadAll": self._excludeDownloadAll
        }

        # Create parser with custom functions
        self.metadata_parser = MetadataParser(self.mapping_config, custom_functions)    

        self.log_file_prefix = "GOE_"
        super().__init__()

        self.profiling_enabled = toolkit.asbool(config.get('tibimport.goettingen_profiling_enabled', False))
        self.profiling_enabled = True
        self._setup_profiling_logger()


    def _setup_profiling_logger(self):
        _profiling_context['enabled'] = self.profiling_enabled
        if not self.profiling_enabled:
            self._profiling_logger = None
            _profiling_context['logger'] = None
            return
        from datetime import date
        profiling_logger = logging.getLogger('tibimport_goe_profiling')
        profiling_logger.setLevel(logging.INFO)
        log_file = os.path.join(self.log_file_path, 'GOE_profiling_' + date.today().strftime("%Y_%m_%d") + '.log')
        fh = logging.FileHandler(log_file)
        fh.setFormatter(logging.Formatter('%(asctime)s | %(message)s'))
        fh.setLevel(logging.INFO)
        profiling_logger.handlers.clear()
        profiling_logger.addHandler(fh)
        self._profiling_logger = profiling_logger
        _profiling_context['logger'] = profiling_logger

    def _profile_log(self, ds_id, step, elapsed_seconds):
        if self.profiling_enabled and self._profiling_logger:
            self._profiling_logger.info(
                f"dataset={ds_id} | step={step} | elapsed={elapsed_seconds:.4f}s"
            )
    ...
    def extend_metadata_from_API(self, api_data):
        ...
        for dataset in api_data:
            ...
            self._profile_log(ds_doi, "step1_api_collection", time.time() - _t0)
            if not response.ok:
    ...
    def get_remote_datasets_paged(self, resumption_token=""):
        ...
        dict_list = []
        for dataset in ds_list:
            dict_list.append(self.parse_goettingen_RECORD_DICT_to_LDM_CKAN_DICT_using_mapping(dataset))

        return {"ds_list": dict_list, "resumptionToken": resumption_token}

    def parse_goettingen_RECORD_DICT_to_LDM_CKAN_DICT_using_mapping(self, ds_dict):
        ds_id = ds_dict.get('identifier', 'unknown')
        _t0 = time.time()
        parsed_dataset = self.metadata_parser.metadata_parser(ds_dict)
        self._profile_log(ds_id, "step2_metadata_mapping", time.time() - _t0)
        return parsed_dataset[0]
    
    # PROFILING HOOKS
    def execute_before_insert_dataset(self, remote_dataset):
        if self.profiling_enabled:
            _profiling_context['insert_start_time'] = time.time()
            _profiling_context['ds_name'] = remote_dataset.get('name', 'unknown')
        return remote_dataset

    def execute_after_insert_dataset(self, remote_dataset):
        # Steps 3 and 4 are logged inside kgcreation's after_create using _profiling_context.
        # Reset here as a safety cleanup in case kgcreation did not run (e.g. non-public dataset).
        if self.profiling_enabled and _profiling_context['insert_start_time'] is not None:
            elapsed = time.time() - _profiling_context['insert_start_time']
            self._profile_log(_profiling_context['ds_name'], "step3_insert_no_kg", elapsed)
        _profiling_context['insert_start_time'] = None
        _profiling_context['ds_name'] = None
        return remote_dataset

    # CUSTOM MAPPING FUNCTIONS
    def _extractDOI(self, value, params):
        if value and "doi.org/" in value:
            return value.replace("https://doi.org/", "").replace("http://doi.org/", "")
        return value
    
    def _extractYear(self, value, params):
        if value and "-" in value:
            return value.split("-")[0]
        
        return value

    def _buildGoettingenName(self, value, params):
        # Extract DOI
    ...
    def _get_goettingen_organization(self, value, params):
        return self._get_goettingen_organization_ckan_dict()
    
    def _processGoettingenCreators(self, value, params):

        authors = []
        for creator in value:
            
            author = {"author": self._get_goettingen_value(creator, ['name']),
                      "givenName": self._get_goettingen_value(creator, ['givenName']),
                      "familyName": self._get_goettingen_value(creator, ['familyName']),
                      "orcid": self._get_goettingen_value(creator, ['orcid'])}
            authors.append(author)
        
        return authors
    
    def _processGOEPublisher(self, value, params):
        if isinstance(value, dict) and "name" in value:
            return [{"publisher": value["name"]}]
        return [] 
    
    def _processGOESubjectAreas(self, value, params):
        if not value:
            return []
    ...
    def _processGOERelatedPublications(self, value, params):
        if not value:
            return []
    ...
    def _processGoettingenKeywords(self, value, params):
        if not value:
            return []
        ...
        for keyword in value:
            ...
            if ',' in tag:
                for t in tag.split(','):
                    t = self._adjust_tag2(t, PERMITTED_CHARS)
                ...
                tag = self._adjust_tag2(tag, PERMITTED_CHARS)
    ...
    def _adjust_tag2(self, tag, permitted_chars):
        tag = tag.replace("/", "-")
...
# '{http://datacite.org/schema/kernel-4}subjects', '{http://datacite.org/schema/kernel-4}subject', '{http://datacite.org/schema/kernel-4}contributors', '{http://datacite.org/schema/kernel-4}contributor', '{http://datacite.org/schema/kernel-4}contributorName', '{http://datacite.org/schema/kernel-4}dates', '{http://datacite.org/schema/kernel-4}date', '{http://datacite.org/schema/kernel-4}language', '{http://datacite.org/schema/kernel-4}resourceType', '{http://datacite.org/schema/kernel-4}alternateIdentifiers', '{http://datacite.org/schema/kernel-4}alternateIdentifier', '{http://datacite.org/schema/kernel-4}alternateIdentifier', '{http://datacite.org/schema/kernel-4}relatedIdentifiers', '{http://datacite.org/schema/kernel-4}relatedIdentifier', '{http://datacite.org/schema/kernel-4}rightsList', '{http://datacite.org/schema/kernel-4}rights', '{http://datacite.org/schema/kernel-4}descriptions', '{http://datacite.org/schema/kernel-4}description']
    ...
    def should_be_updated(self, local_dataset, remote_dataset):
        ...
        result = False
        exclude_in_comparison = ['owner_org', 'license_title', 'organization', 'tags', 'isopen', 'description_text']
