...
from ckanext.tibimport.metadata_parser import MetadataParser
...
class LEUPHANA_ParserProfile(DatasetParser):
    ...
    DataCite published within the DOI using DataCite's API.
    ...
    def __init__(self):
        ...
        # Ex. https://pubdata.leuphana.de/oai/request?verb=ListRecords&metadataPrefix=dim&set=com_123456789_3
        # This url retrieves all identifiers for Research data (Forschungsdaten), Datasets and others, open or not
        self.leuphana_ListRecords_url = 'https://pubdata.leuphana.de/server/oai/request?verb=ListRecords'
        
        # REQUIREMENT: import only Datasets and only open (free to access): 
        # Harverster not allowing sets, filtering should be managed programatically 
        self.leuphana_metadata_schema_response = 'dim'
        ...
        self.ns2 = 'http://www.dspace.org/xmlns/dspace/dim'
        self.ns3 = 'http://www.w3.org/2001/XMLSchema-instance'
        ...
        self.DataCite_api = dataCite_API_Search(self.publisher_name)

        # OAIHarvester — replaces requests.get for OAI-PMH calls in get_datasets_list
        _oai_base = self.leuphana_ListRecords_url.split('?')[0]
        self.harvester = OAIHarvester(
            base_url=_oai_base,
            metadata_prefix=self.leuphana_metadata_schema_response,
        )

        # MetadataParser setup for combined OAI+DataCite dict → CKAN dict mapping
        self._this_file_path = os.path.dirname(os.path.abspath(__file__))
        _mappings_file = os.path.join(self._this_file_path, 'mappings', 'LEU_profile_mapping.json')
        with open(_mappings_file, "r") as f:
            self._leu_mapping_config = json.load(f)
        _leu_custom_functions = {
            "getLEUpublicationYear":    self._getLEUpublicationYear,
            "getLEUfirstTitle":         self._getLEUfirstTitle,
            "getLEUfirstDescription":   self._getLEUfirstDescription,
            "getLEUorganization":       self._getLEUorganization,
            "buildLEUname":             self._buildLEUname,
            "processLEUcreators":       self._processLEUcreators,
            "processLEUsubjectAreas":   self._processLEUsubjectAreas,
            "processLEUtags":           self._processLEUtags,
            "processLEUpublishers":     self._processLEUpublishers,
        }
        self.leu_metadata_parser = MetadataParser(self._leu_mapping_config, _leu_custom_functions)
    ...
    def get_datasets_list(self, ds_list=[], resumption_token=''):
        '''
            Uses OAIHarvester to retrieve a list of datasets page by page.
            OAIHarvester._request handles HTTP, retries and OAI error detection.
        ...
        if not resumption_token:
            params = {
                "verb": "ListRecords",
                "metadataPrefix": self.leuphana_metadata_schema_response,
                "set": "com_123456789_3",
            }
        else:
        ...
        try:
            xml_tree_data = self.harvester._request(params)
        except OAIError as e:
            self.set_log("error_api_data", f"{e.code} - {e.message}")
            return []
        except OAIHarvestError as e:
            self.set_log("error_API", str(e))
            return []
        ...
        resumption_token_data = self._get_leuphana_resumption_token(xml_tree_data)
        if resumption_token_data['resumptionToken']:
            self.get_datasets_list(ds_list, resumption_token_data['resumptionToken'])

        ds_list = self.filter_dataset_type(ds_list)
        ds_list = self.fetch_multiple_json_data_from_datacite(ds_list)
    ...
    def _get_page_of_Datasets(self, url):
        ...
        if not response.ok:
            ...
            obj_tree = ET.fromstring(response.text)
        return  obj_tree
    ...
    def get_remote_datasets_paged(self, resumption_token=''):
        ...
        if xml_tree_data is None:
            return {"ds_list": [], "resumptionToken": ""}
        ...
        # Search for metadata in DataCite
        ds_list = self.fetch_multiple_json_data_from_datacite(ds_list)
    ...
    def fetch_multiple_json_data_from_datacite(self, data):

        result_list = []
        for dataset in data:
            identifiers = dataset.get("dc", {}).get("identifier", [])
            doi = ""
            for id in identifiers:
                if 'doi.org' in id.get("value", ""):
                    doi_split = id['value'].split("/")
                    doi = doi_split[-2]+'/'+doi_split[-1]
            if doi:
                doi_metadata = self.DataCite_api.get_key_doi_metadata(doi)
                dataset["doi_metadata"] = doi_metadata
                dataset['doi'] = doi
    ...
    def is_open_dataset(self, ds_dict):
        # 'rightsList': [{'rights': 'Anonymous'}, {'lang': 'en-US', 'rights': 'Creative Commons Attribution 4.0 International', 
        # 'rightsUri': 'https://creativecommons.org/licenses/by/4.0/legalcode', 'schemeUri': 
        # 'https://spdx.org/licenses/', 'rightsIdentifier': 'cc-by-4.0', 'rightsIdentifierScheme': 'SPDX'},
        license_list = ds_dict.get('doi_metadata', {}).get('rightsList', [])
        is_open = False
        for license in license_list:
            if 'creativecommons.org' in license.get('rightsUri', "") or 'opendatacommons.org' in license.get('rightsUri', ""):
                ds_dict['license_uri'] = license.get('rightsUri', "")
                ds_dict['license'] = license.get('rights', "")
                is_open = True
        return is_open

    def is_dataset_type(self, ds_dict):
         # type_list = {'dc': 'type': [{'value': 'Dataset', 'lang': '-'}]
        ds_types = ds_dict.get('dc', {}).get('type', [])
        for ds_type in ds_types:
            if 'Dataset' in ds_type['value']:
                return True
        #print("\nFALSE DS TYPE:\n", ds_dict)    
        return False
    ...
    def parse_leuphana_XML_result_to_DICT(self, xml_tree_data):
        ...
        Args:
            xml_data (str): XML data as a ElementTree
        ...
        try:
            ...
            # get schema data only first time
            if not self.current_schema_report:
                self.current_schema_report = self._get_schema_report(root)
    ...
    # -------------------------------------------------------------------------
    # MetadataParser-based parsing (combined OAI+DataCite dict → LDM CKAN dict)
    # -------------------------------------------------------------------------

    def parse_leuphana_RECORD_DICT_to_LDM_CKAN_DICT_using_mapping(self, leuphana_dict):
        parsed = self.leu_metadata_parser.metadata_parser(leuphana_dict)
        return parsed[0] if parsed else {}

    # Custom mapping functions for LEU_profile_mapping.json

    def _getLEUpublicationYear(self, value, params):
        if not value or not isinstance(value, list):
            return ''
        return value[0].get('value', '') if value else ''

    def _getLEUfirstTitle(self, value, params):
        if not value or not isinstance(value, list):
            return None
        t = value[0].get('value', '') if value else ''
        return t.capitalize() if t else None

    def _getLEUfirstDescription(self, value, params):
        if not value or not isinstance(value, list):
            return None
        return value[0].get('value', '') if value else None

    def _getLEUorganization(self, value, params):
        return self._get_leuphana_organization_ckan_dict()

    def _buildLEUname(self, value, params):
        if not value or not isinstance(value, str):
            return ''
        name = 'doi-' + value.replace('/', '-').replace('.', '-').lower()
        name = self.dataset_title_prefix + name
        return urllib.parse.quote(name[:100].strip())

    def _processLEUcreators(self, value, params):
        if not value or not isinstance(value, list):
            return None
        authors = []
        for creator in value:
            ...
            for nid in creator.get('nameIdentifiers', []):
                if isinstance(nid, dict) and nid.get('nameIdentifierScheme', '').upper() == 'ORCID':
                    uri = nid.get('nameIdentifier', '')
                    orcid = uri.split('/')[-1] if '/' in uri else uri
                    break
            authors.append({
                'author':     creator.get('name', ''),
                'givenName':  creator.get('givenName', '') or '',
                'familyName': creator.get('familyName', '') or '',
                'orcid':      orcid,
            })
        return authors if authors else None

    def _processLEUsubjectAreas(self, value, params):
        if not value or not isinstance(value, list):
            return None
        result = [
            {'subject_area_name': s.get('value', '')}
            for s in value
            if isinstance(s, dict) and s.get('qualifier') == 'topic' and s.get('value')
        ]
        return result if result else None

    def _processLEUtags(self, value, params):
        if not value or not isinstance(value, list):
            return None
        keywords = [
            s.get('value', '') for s in value
            if isinstance(s, dict) and s.get('qualifier') == 'keyword' and s.get('value')
        ]
        tags = self._fix_leuphana_keywords(keywords)
        return tags if tags else None

    def _processLEUpublishers(self, value, params):
        if not value or not isinstance(value, list):
            return None
        result = [
            {'publisher': item.get('value', '')}
            for item in value
    ...
    def parse_leuphana_RECORD_DICT_to_LDM_CKAN_DICT(self, leuphana_dict):
        ...
        leuphana_metadata = leuphana_dict
        leuphana_metadata_json = leuphana_dict['doi_metadata']

        # idenfier
        identifier_type = "DOI"
        identifier = self._get_leuphana_value(leuphana_metadata, ['doi'])
        datestamp = self._get_leuphana_value(leuphana_dict['header'], ['datestamp'])
        publication_year = self._get_leuphana_value(leuphana_metadata, ['DataCite', 'publicationYear'])
        ...
        # creators
        #print("\n\nDOI:\n", identifier)
        ldm_dict = self._get_leuphana_creators(leuphana_metadata_json, ldm_dict)
        ...
        # rights
        ldm_dict['license_id'] = leuphana_metadata['license_uri']
        ldm_dict['license_title'] = leuphana_metadata['license']
        ...
        ldm_dict = self._get_leuphana_subject_areas_and_keywords(leuphana_metadata, ldm_dict)

        # resource type - The filter doesn's allow other type more than Dataset
        ldm_dict['resource_type'] = 'Dataset'
    ...
    def _get_leuphana_title(self, leuphana_metadata, ldm_dict):
        # 'title': [{'value': 'BNBN 2024: Betriebsrat-Datensatz', 'lang': 'de'}], 
        titles = leuphana_metadata.get('dc', []).get('title', [])
        title_txt = ""

        for title in titles:
            title_txt = title.get("value", "")
            title_lang = title.get("lang", "")
            if title and title_lang == 'en':
                break # take title in english
            
        return title_txt
    
    def _get_leuphana_description(self, leuphana_metadata, ldm_dict):

        description_txt = leuphana_metadata.get('DataCite', {}).get('Description', {})
        for desc in description_txt:
            if 'value' in desc:
                description_txt = desc['value']
                break
    ...
    def _fix_leuphana_keywords(self, keywords_list):
    ...
    def _get_leuphana_publishers(self, leuphana_metadata, ldm_dict):
        # 'Publisher': [{'value': 'Medien- und Informationszentrum, Leuphana Universität Lüneburg', 'lang': '-'}]},
        publisher = leuphana_metadata.get('DataCite', {}).get('Publisher', [])
        if len(publisher)>0:
            publisher = publisher[0]['value']
        
        ldm_dict['publishers'] = [{'publisher': publisher}]

        return ldm_dict

    def _get_leuphana_subject_areas_and_keywords(self, leuphana_metadata, ldm_dict):
        # 'local': 'Subject': [{'value': 'Nachhaltige Transformation', 'qualifier': 'topic', 'lang': '-'}, 
        #                                     {'value': 'Klimaschutz', 'qualifier': 'keyword', 'lang': 'de'}, 
        s_areas = leuphana_metadata.get('local', {}).get('Subject', [])
        s_areas_list = []
        keywords_list = []

        for subject in s_areas:
            s_type = subject.get("qualifier", "")
            if s_type == 'keyword' and subject.get('value',''):
                keywords_list.append(subject['value'])
            if s_type == 'topic' and subject.get('value',''):
                s_areas_list.append({"subject_area_name": subject['value']})
                
        if s_areas_list:
            ldm_dict['subject_areas'] = s_areas_list
        if keywords_list:
            ldm_dict['tags'] = self._fix_leuphana_keywords(keywords_list)    
    ...
    def _get_schema_report(self, obj_tree=None):
        ...
        if obj_tree is None:
            ...
            try:
                ...
                self.set_log("error_API", self.leuphana_ListRecords_url_first_time + " - " + e.__str__())

            if not response.ok:
                self.set_log("error_api_data", self.leuphana_ListRecords_url_first_time)
            else:
                obj_tree = ET.fromstring(response.content)
        ...
        if obj_tree is None:
            ...
            ns0 = '{'+self.ns0+'}'
            ns2 = '{'+self.ns2+'}'
            schema_dataset_data = obj_tree.find(ns0 + 'ListRecords/' + ns0 + 'record/' + ns0 + 'metadata/' + ns2 + 'dim')
    ...
    def should_be_updated(self, local_dataset, remote_dataset):
        ...
        result = False
        exclude_in_comparison = ['owner_org', 'organization', 'subject_areas', 'tags', 'license_title', 'description_text']
        # remote converted
            # 'subject_areas': [{'subject_area_name': 'Nachhaltige Transformation'}], 
            # 'tags': [{'display_name': 'Klimaschutz', 'name': 'Klimaschutz', 'state': 'active', 
        # CKAN package_show
            # "subject_areas":[{"subject_area_name": "Nachhaltige Transformation"}], 
            # "tags": [{"display_name": "Betriebsrat", "id": "dde4271a-d9bd-4b2c-b360-ea4eab9c9014", "name": "Betriebsrat", "state": "active", "vocabulary_id": None}                                                  
