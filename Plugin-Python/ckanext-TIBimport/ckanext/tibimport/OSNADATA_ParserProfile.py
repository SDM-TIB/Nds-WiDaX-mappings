...
class OSNADATA_ParserProfile(DatasetParser):
    ...
    def __init__(self):
        ...
        self.harvester = OAIHarvester(base_url = "https://osnadata.ub.uni-osnabrueck.de/oai",
                                      metadata_prefix = "oai_datacite",)

        # MetadataParser setup for OAI-based mapping
        self._this_file_path = os.path.dirname(os.path.abspath(__file__))
        _mappings_file = os.path.join(self._this_file_path, 'mappings', 'OSN_profile_mapping.json')
        with open(_mappings_file, "r") as f:
            self._osn_mapping_config = json.load(f)
        _osn_custom_functions = {
            "buildOSNname":                 self._buildOSNname,
            "getOSNfirstTitle":             self._getOSNfirstTitle,
            "getOSNfirstDescription":       self._getOSNfirstDescription,
            "extractOSNlicenseId":          self._extractOSNlicenseId,
            "getOSNorganization":           self._getOSNorganization,
            "processOSNcreators":           self._processOSNcreators,
            "processOSNsubjectAreas":       self._processOSNsubjectAreas,
            "processOSNrelatedIdentifiers": self._processOSNrelatedIdentifiers,
            "extractOSNkeywords":           self._extractOSNkeywords,
        }
        self.osn_metadata_parser = MetadataParser(self._osn_mapping_config, _osn_custom_functions)
    ...
    def get_datasets_list(self, ds_list=[], resumption_token=''):
        '''
            Uses the OAIHarvester to retrieve a list of datasets page by page.
            OAIHarvester._request handles HTTP, retries and OAI error detection.
            Record parsing, filtering and enrichment stay exactly as before.
        ...
        # Build OAI-PMH ListRecords params (first page or continuation)
        if not resumption_token:
            params = {"verb": "ListRecords",
                      "metadataPrefix": self.osnaData_metadata_schema_response}
        else:
            params = {"verb": "ListRecords", "resumptionToken": resumption_token}

        # One HTTP request via the harvester (handles retries internally)
        try:
            xml_tree_data = self.harvester._request(params)
        except OAIError as e:
            # Repository returned an OAI-level error element
        ...
        # Schema check on the very first call (pass tree to avoid a second request)
        if not self.current_schema_report:
            self.current_schema_report = self._get_schema_report(xml_tree_data)

        # Parse XML → dicts using the existing parser (keeps dict format intact)
        page_records = self.parse_osnaData_XML_result_to_DICT(xml_tree_data)
        if ds_list:
            ds_list.extend(page_records)
        else:
            ds_list = page_records

        # Follow the resumption token to the next page (recursive, same as before)
        resumption_token_data = self._get_osnaData_resumption_token(xml_tree_data)
        if resumption_token_data['resumptionToken']:
            return self.get_datasets_list(ds_list, resumption_token_data['resumptionToken'])

        # ── Final processing on the last page ──────────────────────────────────
    ...
    def get_remote_datasets_paged(self, resumption_token=''):
        '''
         Uses the OAIHarvester to retrieve ONE page of datasets.
         Returns: {"ds_list": [...CKAN dicts...], "resumptionToken": str | None}
         Call repeatedly, passing the returned token, until resumptionToken is None.
        '''
        self.set_log("infos_searching_ds")
        ...
        try:
            ...
            self.set_log("error_api_data", f"{e.code} - {e.message}")
            return {"ds_list": [], "resumptionToken": None}
        except OAIHarvestError as e:
            self.set_log("error_API", str(e))
            return {"ds_list": [], "resumptionToken": None}
        ...
        ds_list = self._filter_open_oai_records(page["records"])
        self.total_osnaData_datasets += len(ds_list)

        for ds in ds_list:
            citation_license = self._get_citations_and_license_from_API(ds['header']['identifier'])
            ds['metadata']['citation'] = citation_license['citation']
            ds['metadata']['license']  = citation_license['license']

        dict_list = [self.parse_osnaData_OAI_RECORD_to_LDM_CKAN_DICT_using_mapping(ds) for ds in ds_list]

        return {"ds_list": dict_list, "resumptionToken": page["resumptionToken"]}

    def _filter_open_oai_records(self, records):
        result = []
        for ds in records:
            meta = ds.get('metadata', {})
            rights_list = meta.get('rightsList', [])
            is_open = any('openAccess' in (r.get('rightsURI') or '') for r in rights_list)
            resource_type = meta.get('resourceType', {}).get('resourceTypeGeneral', '')
            is_dataset = 'Dataset' in resource_type
            if is_open and is_dataset:
                result.append(ds)
        return result
...
# '{http://datacite.org/schema/kernel-4}subjects', '{http://datacite.org/schema/kernel-4}subject', '{http://datacite.org/schema/kernel-4}contributors', '{http://datacite.org/schema/kernel-4}contributor', '{http://datacite.org/schema/kernel-4}contributorName', '{http://datacite.org/schema/kernel-4}dates', '{http://datacite.org/schema/kernel-4}date', '{http://datacite.org/schema/kernel-4}language', '{http://datacite.org/schema/kernel-4}resourceType', '{http://datacite.org/schema/kernel-4}alternateIdentifiers', '{http://datacite.org/schema/kernel-4}alternateIdentifier', '{http://datacite.org/schema/kernel-4}alternateIdentifier', '{http://datacite.org/schema/kernel-4}relatedIdentifiers', '{http://datacite.org/schema/kernel-4}relatedIdentifier', '{http://datacite.org/schema/kernel-4}rightsList', '{http://datacite.org/schema/kernel-4}rights', '{http://datacite.org/schema/kernel-4}descriptions', '{http://datacite.org/schema/kernel-4}description']
    ...
    def parse_osnaData_XML_RECORD_to_DICT_using_OAIHarvester(self, xml_tree_obj):
        """
        Alternative to parse_osnaData_XML_RECORD_to_DICT().

        Delegates entirely to OAIHarvester.parse_record() instead of the
        hand-written field-by-field XML extraction in this class.
        No OSNADATA-to-CKAN mapping is applied here — the returned dict
        uses the OAIHarvester structure directly.

        Parameters
        ----------
        xml_tree_obj : xml.etree.ElementTree.Element
            A single <record> element (same input as the original method).

        Returns
        -------
        dict  — OAIHarvester format:
            {
                "header": {
                    "identifier":  "doi:10.26249/FK2/48FTWW",
                ...
                "metadata": {
                    ...
                    "identifier":      {"value": "10.26249/...", "identifierType": "DOI"},
                    "creators":        [...],
                    "titles":          [...],
                    "publisher":       {...},
                    "publicationYear": "2024",
                    "resourceType":    {"value": None, "resourceTypeGeneral": "Dataset"},
                    "subjects":        [...],
                    "contributors":    [...],
                    "dates":           [...],
                    "language":        None,
        ...
        """
        return self.harvester.parse_record(xml_tree_obj)
    ...
    # -------------------------------------------------------------------------
    # MetadataParser-based parsing (OAI format → LDM CKAN dict)
    # -------------------------------------------------------------------------

    def parse_osnaData_OAI_RECORD_to_LDM_CKAN_DICT_using_mapping(self, oai_record_dict):
        parsed = self.osn_metadata_parser.metadata_parser(oai_record_dict)
        result = parsed[0] if parsed else {}
        meta = oai_record_dict.get('metadata', {})
        if 'license' in meta:
            result['license_id'] = meta['license']
        ddc_areas = self._extractOSNcitationDDC(meta.get('citation', {}))
        if ddc_areas:
            result.setdefault('subject_areas', []).extend(ddc_areas)
        return result

    # Custom mapping functions for OSN_profile_mapping.json

    def _buildOSNname(self, value, params):
        if not value or not isinstance(value, dict):
            return ''
        identifier_type = value.get('identifierType', '')
        identifier_value = value.get('value', '')
        name = identifier_type + '-' + identifier_value
        for char in ' /.':
    ...
    def _processOSNcreators(self, value, params):
        ...
        for creator in value:
            if not isinstance(creator, dict):
                continue
            orcid = ''
            for name_id in creator.get('nameIdentifiers', []):
                if isinstance(name_id, dict) and name_id.get('nameIdentifierScheme', '').upper() == 'ORCID':
                    orcid = name_id.get('value', '')
                    break
            authors.append({
                'author':     creator.get('creatorName', {}).get('value', ''),
    ...
    def _getOSNfirstTitle(self, value, params):
        if not value or not isinstance(value, list):
            return None
        title = value[0].get('value', '') if value else ''
        return title.capitalize() if title else None

    def _getOSNfirstDescription(self, value, params):
        if not value or not isinstance(value, list):
            return None
        return value[0].get('value', '') if value else None

    def _extractOSNlicenseId(self, value, params):
        if not value or not isinstance(value, list):
            return ''
        for rights in value:
            if not isinstance(rights, dict):
                continue
            rights_id = rights.get('rightsIdentifier', '')
            if rights_id:
                return rights_id
            rights_uri = rights.get('rightsURI', '') or ''
            if rights_uri in open_access_licenses:
                return open_access_licenses[rights_uri]['title']
           
        return ''

    def _getOSNorganization(self, value, params):
        return self._get_osnaData_organization_ckan_dict()

    def _processOSNsubjectAreas(self, value, params):
        if not value or not isinstance(value, list):
            return None
        result = []
        for subject in value:
            if not isinstance(subject, dict):
                continue
            name = subject.get('value', '')
            if name:
                result.append({'subject_area_additional': '', 'subject_area_name': name})
        return result if result else None

    def _processOSNrelatedIdentifiers(self, value, params):
        if not value or not isinstance(value, list):
            return None
        result = []
        for rel_id in value:
    ...
    def _extractOSNcitationDDC(self, citation):
        result = []
        for field in citation.get('fields', []):
            if field.get('typeName') == 'ddc':
                for value in field.get('value', []):
                    result.append({'subject_area_additional': '', 'subject_area_name': value})
        return result

    def _extractOSNkeywords(self, value, params):
        return self._extractOSNcitationKeywords(value) or None

    def _extractOSNcitationKeywords(self, citation):
        tag_list = []
        for field in citation.get('fields', []):
            if field.get('typeName') == 'keyword':
                for keyword in field.get('value', []):
                    keyword_txt = keyword.get('keywordValue', {}).get('value', '')
                    if keyword_txt:
                        tag = self._adjust_tag(keyword_txt)
                        if tag:
                            tag_list.append({'display_name': tag, 'name': tag,
    ...
    def should_be_updated(self, local_dataset, remote_dataset):
        ...
        result = False
        exclude_in_comparison = ['owner_org', 'license_title', 'organization', 'subject_areas', 'tags', 'description_text']
