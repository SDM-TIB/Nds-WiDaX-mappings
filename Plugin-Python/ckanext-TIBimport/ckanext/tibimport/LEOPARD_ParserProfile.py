...
class leoPARD_ParserProfile(DatasetParser):
    ...
    def __init__(self):
        ...
        # OAIHarvester — replaces direct requests.get in get_datasets_list
        self.harvester = OAIHarvester(
            base_url=self.leoPARD_ListRecords_url.split('?')[0],
            metadata_prefix=self.leoPARD_metadata_schema_response,
        )

        # MetadataParser setup for OAI-based mapping
        self._this_file_path = os.path.dirname(os.path.abspath(__file__))
        _mappings_file = os.path.join(self._this_file_path, 'mappings', 'LEO_profile_mapping.json')
        with open(_mappings_file, "r") as f:
            self._leo_mapping_config = json.load(f)
        _leo_custom_functions = {
            "buildLEOname":                 self._buildLEOname,
            "getLEOfirstTitle":             self._getLEOfirstTitle,
            "processLEOdescriptions":       self._processLEOdescriptions,
            "extractLEOlicenseId":          self._extractLEOlicenseId,
            "extractLEOlicenseTitle":       self._extractLEOlicenseTitle,
            "getLEOorganization":           self._getLEOorganization,
            "processLEOcreators":           self._processLEOcreators,
            "processLEOsubjectAreas":       self._processLEOsubjectAreas,
            "processLEOtags":               self._processLEOtags,
            "processLEOresourceType":       self._processLEOresourceType,
            "processLEOrelatedIdentifiers": self._processLEOrelatedIdentifiers,
        }
        self.leo_metadata_parser = MetadataParser(self._leo_mapping_config, _leo_custom_functions)
    ...
    def get_datasets_list(self, ds_list=[], resumption_token=''):
        ...
        if not resumption_token:
            params = {
                "verb": "ListRecords",
                "metadataPrefix": self.leoPARD_metadata_schema_response,
                "set": "GENRE:" + self.leoPARD_GENRE,
            }
        else:
        ...
        resumption_token_data = self._get_leoPARD_resumption_token(xml_tree_data)
...
# '{http://datacite.org/schema/kernel-4}subjects', '{http://datacite.org/schema/kernel-4}subject', '{http://datacite.org/schema/kernel-4}contributors', '{http://datacite.org/schema/kernel-4}contributor', '{http://datacite.org/schema/kernel-4}contributorName', '{http://datacite.org/schema/kernel-4}dates', '{http://datacite.org/schema/kernel-4}date', '{http://datacite.org/schema/kernel-4}language', '{http://datacite.org/schema/kernel-4}resourceType', '{http://datacite.org/schema/kernel-4}alternateIdentifiers', '{http://datacite.org/schema/kernel-4}alternateIdentifier', '{http://datacite.org/schema/kernel-4}alternateIdentifier', '{http://datacite.org/schema/kernel-4}relatedIdentifiers', '{http://datacite.org/schema/kernel-4}relatedIdentifier', '{http://datacite.org/schema/kernel-4}rightsList', '{http://datacite.org/schema/kernel-4}rights', '{http://datacite.org/schema/kernel-4}descriptions', '{http://datacite.org/schema/kernel-4}description']
    ...
    def parse_leoPARD_XML_RECORD_to_DICT_using_OAIHarvester(self, record_el):
        """Delegate to OAIHarvester.parse_record() to produce the standard OAI dict format."""
        return self.harvester.parse_record(record_el)

    # -------------------------------------------------------------------------
    # MetadataParser-based parsing (OAI format → LDM CKAN dict)
    # -------------------------------------------------------------------------

    def parse_leoPARD_OAI_RECORD_to_LDM_CKAN_DICT_using_mapping(self, oai_record_dict):
        parsed = self.leo_metadata_parser.metadata_parser(oai_record_dict)
        return parsed[0] if parsed else {}

    # Custom mapping functions for LEO_profile_mapping.json

    def _buildLEOname(self, value, params):
        if not value or not isinstance(value, dict):
            return ''
        name = value.get('identifierType', '') + '-' + value.get('value', '')
        for c in ' /.':
            name = name.replace(c, '-')
        name = name.lower()
    ...
    def _getLEOfirstTitle(self, value, params):
        if not value or not isinstance(value, list):
            return None
        t = value[0].get('value', '') if value else ''
        return t.capitalize() if t else None

    def _processLEOdescriptions(self, value, params):
        if not value or not isinstance(value, list):
            return None
        parts = [d.get('value', '') for d in value if isinstance(d, dict) and d.get('value')]
        return '\r\n'.join(parts) if parts else None

    def _extractLEOlicenseId(self, value, params):
        if not value or not isinstance(value, list):
            return ''
        for rights in value:
            if not isinstance(rights, dict):
                continue
            rid = rights.get('rightsIdentifier', '')
            if rid:
                return rid
        return ''

    def _extractLEOlicenseTitle(self, value, params):
        if not value or not isinstance(value, list):
            return ''
        return value[0].get('value', '') if value else ''

    def _getLEOorganization(self, value, params):
        return self._get_leoPARD_organization_ckan_dict()

    def _processLEOcreators(self, value, params):
        if not value or not isinstance(value, list):
            return None
        authors = []
        for creator in value:
            if not isinstance(creator, dict):
                continue
            orcid = next(
                (n.get('value', '') for n in creator.get('nameIdentifiers', [])
                 if isinstance(n, dict) and n.get('nameIdentifierScheme', '').upper() == 'ORCID'),
                ''
            )
            authors.append({
    ...
    def _processLEOsubjectAreas(self, value, params):
        if not value or not isinstance(value, list):
            return None
        result = []
        for subject in value:
            if not isinstance(subject, dict):
                continue
            if subject.get('subjectScheme', '') == 'ddc':
                ddc_id = self._adjust_ddc_id(subject.get('value', ''))
                result.append({'subject_area_name': ddc_id + ' ' + self._get_DDC_value_from_id(ddc_id)})
        return result if result else None

    def _processLEOtags(self, value, params):
        if not value or not isinstance(value, list):
            return None
        keywords = [
            s.get('value', '') for s in value
            if isinstance(s, dict) and not s.get('subjectScheme') and s.get('value')
        ]
        tags = self._fix_leoPARD_keywords(keywords)
        return tags if tags else None

    def _processLEOresourceType(self, value, params):
        if not value or not isinstance(value, dict):
            return None
        general  = value.get('resourceTypeGeneral', '')
        specific = value.get('value', '')
        if general and specific:
            return general + ' - ' + specific
        return general or specific or None

    def _processLEOrelatedIdentifiers(self, value, params):
        if not value or not isinstance(value, list):
            return None
        result = []
        for rel_id in value:
    ...
    def should_be_updated(self, local_dataset, remote_dataset):
        ...
        result = False
        exclude_in_comparison = ['owner_org', 'license_title', 'organization', 'subject_areas', 'description_text']
