# pytest --ckan-ini=test.ini ckanext/tibimport/tests/test_LUH_profile.py -s
...
from LUH_Profile_Mocks import local_organization_data, local_dataset_data, luh_api_package_list_no_results,\
    ...
    local_dataset_after_old_mapping, local_dataset_after_old_mapping_and_adjusted_fixed
...
#     print("LUH PACKAGE SHOW:/n", response.content)
#     assert response.ok
...
def test_luh_api_package_show_ok():
    ...
    with patch('ckanext.tibimport.logic2.requests.post') as mock_get:
        ...
        res_dict = obj.get_dataset_data(ds_name)
        print("\n\nAPI response package_show with results:\n", res_dict)
...
def test_mapping_using_metadata_parser_mock():
    ...
    with patch('ckanext.tibimport.logic2.requests.post') as mock_get:
        ...
        res_dict = obj.get_dataset_data(ds_name)
        res_dict = obj.adjust_dataset_dict_with_mapping(res_dict)
        #res_dict = obj.adjust_dataset_dict(res_dict)
        #res_dict = obj._process_authors(res_dict)
        print("\n\nPACKAGE MAPPED:\n", res_dict)

    assert res_dict == local_dataset_after_old_mapping_and_adjusted_fixed
